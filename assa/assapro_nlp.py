...
class WindowAttention_sparse(nn.Module):
    def __init__(
        ...
        trunc_normal_(self.relative_position_bias_table, std=0.02)  # 初始化相对位置偏置
        ...
        # 动态权重，用于平衡SSA和DSA的混合
        self.dynamic_weights = nn.Parameter(
            torch.ones(2)
        )  # 动态学习的权重参数，控制SSA和DSA的比例
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        # 使用动态权重来平衡SSA和DSA
        w1 = torch.exp(self.dynamic_weights[0]) / torch.sum(
            torch.exp(self.dynamic_weights)
        )
        w2 = torch.exp(self.dynamic_weights[1]) / torch.sum(
            torch.exp(self.dynamic_weights)
        )

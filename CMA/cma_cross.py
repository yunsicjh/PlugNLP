...
if __name__ == "__main__":
    ...
    # 创建图像和文本特征图张量
    img_feat = torch.randn(batch_size, channels, height, width)  # 输入图像特征图
    text_feat = torch.randn(batch_size, channels, height, width)  # 输入文本特征图

    # 初始化交叉模态注意力层
    cross_attention = CrossModelAtt(feature_dim=channels, height=height, width=width)

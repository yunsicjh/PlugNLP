...
class LightBiMAFusion(nn.Module):
    ...
    def forward(self, img_feat, txt_feat):
        ...
        # 上采样恢复
        fusion_img_up = F.interpolate(
            fusion_img, size=target_size, mode="bilinear", align_corners=False
        )
        fusion_txt_up = F.interpolate(
            fusion_txt, size=target_size, mode="bilinear", align_corners=False
        )
...
if __name__ == "__main__":
    ...
    img_input = torch.randn(B, C_img, H, W)
    txt_input = torch.randn(B, C_txt, H, W)

    model = LightBiMAFusion(
        img_channels=C_img, txt_channels=C_txt, mid_channels=64, attn_size=64
    )
